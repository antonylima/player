...
let audioContext;
let panNode;
let balanceValue = Number(balance.value);
...
const initAudioContext = () => {
  ...
  try {
    ...
    panNode.pan.value = balanceValue;
    sourceNode.connect(panNode).connect(audioContext.destination);
  } catch (error) {
    audioContext = null;
    panNode = null;
    balance.disabled = true;
    balance.title = "Balance indisponível para esta fonte.";
  }
};
...
balance.addEventListener("input", (event) => {
  balanceValue = Number(event.target.value);
  if (panNode) {
    panNode.pan.value = balanceValue;
  }
});
